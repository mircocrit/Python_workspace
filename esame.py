...
s_x2=0
for i in range(0, len(w_k)):
    print('{:1.3f}'.format(np.power((w_k[i]-m_x),2)))
    
    print('{:1.3f}'.format(np.power((w_k[i]-m_x),2)*Nk[i]))
    print('')
...
for i in range(0, len(w_k)):
    ...
    #print('{:1.3f}'.format(np.power((w_k[i]),2)*w_k[i]))
    #print('')
    s_x2+=((np.power((w_k[i]),2))*pk[i])
#s_x2=(1/tot)*s_x2
s_x2=s_x2-np.power(m_x,2)
"""
